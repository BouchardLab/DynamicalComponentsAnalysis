...
def cross_validate_toeplitz_fit_simple(X_with_lags, ps, pt, num_folds=10):

    fold_size = int(np.floor(len(X_with_lags)/num_folds))
    d = X_with_lags.shape[1]
    P = build_P(pt)
    max_rank = np.minimum(2*pt - 1, ps**2)
    log_likelihood_vals = np.zeros((num_folds, max_rank))
    ...
    for cv_iter in range(num_folds):
        ...
        R_C = pv_rearrange(cov_train, ps, pt)
        U, s, Vt = scipy.linalg.svd(P.dot(R_C))

        for r_idx in range(max_rank):

            r = r_idx + 1
            svd_stuff = U[:, :r].dot(np.diag(s[:r])).dot(Vt[:r, :])
            cov_reg_train = pv_rearrange_inv(P.T.dot(svd_stuff), ps, pt)
            cov_reg_train_inv = np.linalg.inv(cov_reg_train)
            log_det_cov_reg_train = np.linalg.slogdet(cov_reg_train)[1]
            log_likelihood = -0.5*num_samples*(d*np.log(2*np.pi) + log_det_cov_reg_train + np.trace(np.dot(cov_reg_train_inv, cov_test)))
...
""" 
def regularize_cov_simple(X_with_lags, ps, pt, num_folds=10):

    mat_size = np.minimum(2*pt - 1, ps**2)
    log_likelihood_vals = np.zeros(mat_size)

    for r_idx in range(mat_size):

        r = r_idx + 1

        log_likelihood_vals_cv, rank_vals_cv, sparsity_vals_cv = cross_validate_toeplitz_fit_simple(X_with_lags, ps, pt, lambda_L, lambda_S,
                                                                                             num_folds=num_folds, tau=tau, tol=tol, max_iter=max_iter,
                                                                                             stop_cond_interval=stop_cond_interval)
